...
Expected output (the treatment mean is a robust GRIM failure — 18.75 is
unreachable by any integer sum over n = 22 under *either* round-half or
truncation rounding, so it fails under the default `convention="both"`):

```
FAIL (2)
------------------------------------------------------------
  [grim] Q-scale / treatment  (Brown & Heathers 2017)
      FAIL: mean 18.75 x n=22 = 412.5000; nearest achievable means are 18.7273 and 18.7727 — impossible for integer-granular data at n=22 under round-half and truncation rounding
  [grimmer] Q-scale / treatment  (Anaya 2016)
      FAIL: GRIM fails first — mean 18.75 is impossible for integer-granular data at n=22 under round-half and truncation rounding
...

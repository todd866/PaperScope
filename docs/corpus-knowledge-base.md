...
**Status: shipped (core).** `paperscope.systematic_review.rater_compare` compares
two raters' screening/extraction JSONL field-by-field: per-field agreement
counts + percent agreement, Cohen's kappa (Cohen 1960) for a categorical field,
and a per-record disagreement listing for adjudication. It handles missing
records, list-valued fields (set comparison), and case/whitespace normalization.
CLI: `rater-compare`. The richer surfaces below (prose-distance, cluster-level
patterns, 2+ raters) remain roadmap.
...
**Status: shipped (first bundle).** `paperscope.systematic_review.knowledge_base`
exports a self-contained bundle from a review corpus dir:

```text
kb/
  paper-cards.jsonl   one card per included record: identity, screening
                      decision-of-record, charted fields, quality flags
  clusters.json       cards grouped by a configurable charted field
  manifest.json       counts, provenance, generated-from paths, schema version
```

Screening/extraction are loaded through `validate.effective`, so human
overrides are the decisions-of-record on every card (a flip changes card
membership; a `corrected_fields` override changes a charted value). Pure
functions (`build_paper_cards`, `build_clusters`, `build_manifest`,
`build_bundle`) plus a `knowledge-base` CLI subcommand. Generic: the caller
supplies which fields land on a card, which field clusters, and which fields
are quality flags.

The fuller target below (a `claims.jsonl` / `source-manifest.jsonl` /
`public-index.json` split, consumed by the static HTML builder or a richer
portal) remains roadmap:
...
3. ✅ **Shipped.** Add `systematic_review.rater_compare` for structured field-level disagreement.
4. Add paper-in-corpus evaluation fields to paper cards.
5. ✅ **Shipped.** Add `systematic_review.knowledge_base` exporter (paper-cards.jsonl + clusters.json + manifest.json).

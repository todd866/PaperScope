...
This is a *generalised* version of a working scoping-review pipeline. That
review remains its first user and its regression dogfood; everything about
the particular question lives in an example config, not in the code.
...
The audit/override half is implemented by the `validate` layer (see
`validate.md`): the AI self-audits, a human adjudicates the flagged decisions
in an HTML workbook, and `validate reconcile` writes append-only
`validation-overrides.jsonl`. Those overrides are *consumed*, not archived —
`validate.effective` folds them into the decisions-of-record that `prisma`,
`aggregate`, and `build-site` read, and a requeued re-screen carries human
adjudications forward rather than overwriting them.
...
pilot review ran this with parallel Claude agents — that orchestration lives in
the caller, not here.
...
## Acquisition and institutional access — a caution

The `acquire` pipeline pulls open-access PDFs via Unpaywall and writes an
EZProxy queue for the paywalled tail; the `browser-harvest` command then
drives a real browser through the institution's subscriptions to download
that tail.

**Automated bulk retrieval through an institutional subscription is the
highest institutional-risk feature in this module.** Publisher license
agreements commonly prohibit systematic or automated downloading; publishers
monitor for it, and the standard escalation is to cut off the *institution's*
access — the consequences land on your university library, not just on you.
The harvester also presents a regular-Chrome user agent, so a publisher's
bot detection will not distinguish it from a human reader; that makes the
license question yours to answer, not theirs to enforce softly.

Whether a given review's tail (tens to a few hundred papers a human reviewer
would otherwise fetch by hand) falls within fair use of the subscription is
the operator's call, and institutions differ. Before running at scale:

- check your library's e-resources policy or ask the library directly;
- keep concurrency conservative — the default of 4 parallel pages is an
  upper bound, not a target;
- pace requests (`--inter-paper-delay 5` or more) and group by publisher
  (`--group-by-publisher`) so the load looks like reading, not scraping;
- prefer the open-access and manual routes for anything marginal.

This mirrors the boundary treatment last-resort acquisition gets in
`paperscope.ingest.shadow_library` (stubbed in the public build): paperscope
states the risk honestly; the deployment decision belongs to the operator.

...
# Canonical GRIM FAIL: 18.75 * 22 = 412.5 — a ROBUST impossibility, unreachable
# under EITHER round-half OR truncation (nearest achievable 18.7273 / 18.7727,
# and [18.75, 18.76) admits no integer sum), so it FAILs under the default
# convention="both".  Control mean 17.62 * 21 = 370.02 → sum 370 works (PASS).
SYNTHETIC_TABLE = {
    ...
    "variables": [
        {"name": "Q-scale",
         "groups": [
             {"name": "treatment", "n": 22, "mean": "18.75", "sd": "9.41"},
...
SMALL_TABLE = {
    "variables": [
        # granularity 1 is declared so GRIM applies (an undeclared variable is
        # skipped as UNDETERMINED); 18.75 * 22 = 412.5 is a robust GRIM FAIL
        # under both round-half and truncation.
        {"name": "Q-scale", "granularity": 1,
         "groups": [{"name": "treatment", "n": 22, "mean": "18.75",
                     "sd": "9.41"}]},
    ],
}

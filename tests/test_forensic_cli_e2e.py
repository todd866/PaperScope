...
def test_table_mode_finds_grim_fail():
    ...
    # The classic GRIM failure: mean 18.75 is impossible for n = 22 under
    # BOTH round-half and truncation (a robust, convention-independent FAIL).
    grim_fails = [f for f in report["findings"]
                  if f["check"] == "grim" and f["verdict"] == "FAIL"]
    assert grim_fails, report["summary"]
    assert any("18.75" in str(f["inputs"].get("mean")) for f in grim_fails)

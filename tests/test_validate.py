...
def test_workbook_screening_has_corrected_decision_select():
    # The human's corrected value must be data, not a free-text note: screening
    # workbooks carry a corrected-decision select that the JS exports.
    html = build_workbook(DECISIONS, RECORDS, default_rubric())
    assert html.count('class="corr-dec"') == 2
    for opt in ("include", "exclude", "maybe"):
        assert f'value="{opt}"' in html
    assert "corrected_decision" in html  # exported as structured state


def test_workbook_extraction_has_corrected_fields_input():
    extraction = [{"record_id": "301", "study_design": "cohort"}]
    html = build_workbook(extraction, {}, default_rubric())
    assert 'class="corr-fields"' in html
    assert "corrected_fields" in html  # exported as structured state
    assert 'class="corr-dec"' not in html  # no decision select on extraction rows


def test_reconcile_carries_corrected_values():
    export = {
        "101": {"reviewed": True, "flip": True, "corrected_decision": "exclude",
                "note": "wrong population"},
        "102": {"reviewed": True, "flip": True,
                "corrected_fields": '{"study_design": "rct"}'},
    }
    overrides, requeue = reconcile(DECISIONS, export)
    o101 = next(o for o in overrides if o["record_id"] == "101")
    assert o101["corrected_decision"] == "exclude"
    o102 = next(o for o in overrides if o["record_id"] == "102")
    assert o102["corrected_fields"] == {"study_design": "rct"}  # JSON parsed
    # the requeue rows carry the corrected decision forward too
    r101 = next(r for r in requeue if r["record_id"] == "101")
    assert r101["corrected_decision"] == "exclude"


def test_reconcile_keeps_unparseable_corrected_fields_as_raw():
    overrides, _ = reconcile(
        DECISIONS, {"101": {"flip": True, "corrected_fields": "not json"}}
    )
    o = overrides[0]
    assert o["corrected_fields"] is None
    assert o["corrected_fields_raw"] == "not json"  # kept, not silently dropped

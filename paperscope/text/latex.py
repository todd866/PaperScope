...
def clean_latex(text: str) -> str:
    ...
    """
    # Remove citation/ref/label/url commands entirely BEFORE stripping
    # comments: a raw % inside \url{...}/\href{...} is part of the URL, not
    # a comment, and must not truncate the rest of the line.
    text = re.sub(r"\\(?:cite\w*|ref|eqref|label|url|href)\{[^}]*\}", " ", text)
    # Strip unescaped % comments so commented-out draft text never reaches
    # the cleaned output that feeds embeddings
    text = re.sub(r"(?<!\\)%.*", " ", text)
...
def clean_plaintext(text: str) -> str:
    ...
    # De-hyphenate PDF soft-wrapped words only: a letter before the hyphen
    # and a lowercase letter after the break.  An unconstrained rule merged
    # numeric ranges split across lines ('12-\n15' -> '1215').

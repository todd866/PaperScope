...
so trailing zeros survive for decimal-place inference):

    {
      ...
      "scale": {"lo": 1, "hi": 7, "granularity": 1,
                "convention": "both"},   # granularity: declaring it marks the
                                         # table's variables integer-valued
                                         # (licenses GRIM); lo/hi alone only
                                         # feed quick_sd_check.  convention:
                                         # "both"|"round"|"truncate" (GRIM)
      "variables": [
        {"name": "Q-scale", "dp": 2,                # dp optional (else inferred
                                                # column-wise from strings)
         "granularity": 1,              # per-variable; overrides scale.  When
                                        # NEITHER this nor scale.granularity is
                                        # set, GRIM/GRIMMER are SKIPPED as
                                        # UNDETERMINED (never assume 1 for a
                                        # continuous measure like weight/BMI)
         "grim": true,                  # set false to opt a variable out
         "convention": "both",          # per-variable GRIM rounding convention
         "groups": [{"name": "treatment", "n": 22,
                     "mean": "18.75", "sd": "9.41"}, ...],
         "baseline_p": 0.83,                    # collected for Carlisle
         "baseline_p_type": "continuous"}       # "categorical"|"continuous";
                                                # if given, Carlisle splits by
                                                # type instead of pooling
      ],
      "percentages": [
      ...
      "tests": [
        # reported_p may be a STRING preserving operator + precision
        # ("<0.001", "0.03") or a float (back-compat); it is compared via
        # the shared reported_stats.reported_p_verdict machinery.
...
def run_table_checks(table: dict, source: str = '') -> dict:
    ...
    # A table scale only marks its variables as integer-valued when it
    # DECLARES an explicit granularity (a Likert/count scale).  Bare lo/hi
    # give quick_sd_check its bounds but do NOT license integer-GRIM.
    scale_granularity = _num(scale_spec.get("granularity"))
    scale_convention = scale_spec.get("convention")
    ...
    # baseline entries carry an optional variable type so Carlisle can split
    # categorical vs continuous instead of force-pooling.
    baseline_entries: List[tuple] = []
    for var in _entries(table, "variables"):
        ...
        # Integer-granularity gate (cardinal rule: don't assume granularity=1
        # for an honest continuous measure like weight/BMI — that would brand
        # a valid mean impossible).  GRIM/GRIMMER run only when the variable
        # declares a granularity, OR the table scale declares one; otherwise
        # they are SKIPPED as UNDETERMINED.  A variable can also opt out with
        # "grim": false.
        grim_optout = var.get("grim") is False
        var_gran = _num(var.get("granularity"))
        if var_gran is not None:
            eff_gran, gran_known = var_gran, True
        elif scale_granularity is not None:
            eff_gran, gran_known = scale_granularity, True
        else:
            eff_gran, gran_known = None, False
        run_gran = gran_known and not grim_optout
        convention = var.get("convention") or scale_convention or "both"
        ...
        for g in groups:
            ...
            if mean is not None and n is not None:
                if run_gran:
                    findings.append(_safe(
                        "grim", grim, target,
                        {"mean": mean, "n": n, "scale": eff_gran,
                         "dp": dp_mean, "convention": convention},
                        mean=_as_str(mean), n=n, scale=eff_gran, dp=dp_mean,
                        convention=convention,
                    ))
                elif not grim_optout:
                    findings.append(finding_from_result(
                        "grim",
                        {'detail': (f"UNDETERMINED: {target}: integer "
                                    f"granularity unknown (no per-variable or "
                                    f"scale granularity declared) — cannot "
                                    f"apply GRIM to a possibly-continuous "
                                    f"measure")},
                        target, {"mean": mean, "n": n}))
            if mean is not None and sd is not None and n is not None:
                if run_gran:
                    findings.append(_safe(
                        "grimmer", grimmer, target,
                        {"mean": mean, "sd": sd, "n": n, "scale": eff_gran,
                         "convention": convention},
                        mean=_as_str(mean), sd=_as_str(sd), n=n,
                        scale=eff_gran, dp_mean=dp_mean, dp_sd=dp_sd,
                        convention=convention,
                    ))
                elif not grim_optout:
                    findings.append(finding_from_result(
                        "grimmer",
                        {'detail': (f"UNDETERMINED: {target}: integer "
                                    f"granularity unknown — cannot apply "
                                    f"GRIMMER to a possibly-continuous "
                                    f"measure")},
                        target, {"mean": mean, "sd": sd, "n": n}))
            sd_f = _num(sd)
            ...
            if sd_f is not None and have_bounds and n is not None:
                findings.append(_safe(
                    ...
                    {"sd": sd_f, "n": n, "lo": lo, "hi": hi, "dp_sd": dp_sd},
                    sd=_as_str(sd), n=n, lo=lo, hi=hi, label=target,
                    dp_sd=dp_sd,
                ))
        ...
        if bp is not None:
            baseline_entries.append((bp, var.get("baseline_p_type")))

    # ── Carlisle across ALL collected baseline p-values (needs >= 3) ──
    # If any variable declared a type (categorical/continuous), pass typed
    # tuples so the library splits by type (it warns against pooling) and
    # emit one finding per stratum; otherwise pool (back-compat).
    if len(baseline_entries) >= 3:
        if any(t for _, t in baseline_entries):
            typed = [(p, t or "unspecified") for p, t in baseline_entries]
            res = carlisle_stouffer_fisher(
                typed, label="Table 1 baseline p-values")
            for sub in (res if isinstance(res, list) else [res]):
                vt = sub.get('variable_type', '')
                tgt = (f"Table 1 baseline p-values ({vt})" if vt
                       else "Table 1 baseline p-values")
                findings.append(finding_from_result(
                    "carlisle_stouffer_fisher", sub, tgt,
                    {"p_values": [p for p, _ in typed], "typed": True}))
        else:
            baseline_ps = [p for p, _ in baseline_entries]
            findings.append(_safe(

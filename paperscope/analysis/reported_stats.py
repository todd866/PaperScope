...
def parse_reported_p(reported_p) -> Tuple[str, Optional[float], str]:
    """
    Parse a reported p into (operator, value, printed-string).

    Accepts a float (back-compat — treated as "= value" at the value's
    printed precision) or a string carrying an optional comparison
    operator and an optional leading "p": "0.03", "<0.001", "p < .001",
    "= .02", "> .05", "<=0.05", "≤ .05", "ns".  Operator "≈" is normalised
    to "=", "≤"/"≥" to "<="/">=".  Trailing significance markers
    (*, **, †, ‡) are stripped before parsing, so "0.03*" is "= 0.03".

    Returns ("="|"<"|">"|"<="|">="|"ns", float|None, str).  The string
    preserves the printed digits so callers can recover the reported
    precision.
    """
    if reported_p is None:
        return '=', None, ''
    if isinstance(reported_p, bool):
        return '=', None, str(reported_p)
    if isinstance(reported_p, (int, float)):
        s = f"{reported_p}"
        return '=', float(reported_p), s
    s = str(reported_p).strip()
    # Trailing significance stars/daggers are decoration, not data
    s = re.sub(r'[\s]*[*†‡]+$', '', s)
    low = s.lower().replace('.', '').replace(' ', '')
    if low in ('ns', 'pns', 'ns.', 'ns'):
        return 'ns', None, s
    m = re.match(
        r'^[pP]?\s*(?P<op><=|>=|[=<>≈≤≥])?\s*(?P<num>[-−]?[0-9.]+)\s*$', s)
    if not m:
        return '=', None, s
    op = m.group('op') or '='
    op = {'≈': '=', '≤': '<=', '≥': '>='}.get(op, op)
...
def reported_p_verdict(p_calc: float, reported_p) -> dict:
    """
    Compare an exactly-recomputed (point) p-value against a reported p
    that may carry a comparison operator and a display precision.

    This is the shared p-consistency judgement for the table-mode
    recomputation checks (t-test, ANOVA, chi-squared), so operator and
    decision-flip semantics live in ONE place (here) rather than being
    duplicated as a crude ratio in each check.

    Table-mode semantics (deliberately conservative — the recomputed p is
    derived from *rounded* means/SDs, so it carries its own uncertainty):
      - PASS: the recomputed p is consistent with the reported claim
        (operator honoured; for "=" a lenient ×10 ratio band absorbs the
        input rounding) AND the alpha = .05 significance decision does not
        flip;
      - FLAG: inconsistent, OR the significance decision flips (a reported
        "significant" that recomputes non-significant, or vice versa).
    A p mismatch is never a table-mode FAIL: recomputation from rounded
    summary stats cannot *prove* impossibility.

    Returns a dict: {'verdict', 'reported_op', 'reported_p', 'consistent',
    'decision_flip', 'note'}.

    Crash-proof by contract: malformed input (a non-numeric `p_calc`, an
    unparseable reported string) yields verdict 'UNDETERMINED' — a parsing
    problem is never an exception and never hardens into a judgement.
    """
    def _undetermined(note):
        return {
    ...
    # Guard the recomputed side: any non-numeric p_calc is a caller/input
    # problem, not evidence about the paper.
    try:
        p_calc = float(p_calc)
    except (TypeError, ValueError):
        return _undetermined(
            f"could not interpret the recomputed p ({p_calc!r}) — cannot check")
    if math.isnan(p_calc):
        return _undetermined("recomputed p is NaN — cannot check")

    op, pval, pstr = parse_reported_p(reported_p)
    # An unparseable non-empty reported string (parse fell back to
    # ('=', None, s)) cannot be judged — UNDETERMINED, not a silent PASS.
    if op != 'ns' and pval is None and str(pstr).strip():
        return _undetermined(
            f"could not parse the reported p ({reported_p!r}) — cannot check")

    half = 0.5 * 10 ** -_dp_from_str(pstr) if pstr else 0.0

    # Consistency of the recomputed point p with the reported claim.
    # '<='/'>=' are inclusive bounds; with the half-ulp slack they judge
    # the same as '<'/'>'.
    if op == 'ns':
        consistent = p_calc > ALPHA - _EPS
    elif op in ('<', '<='):
        consistent = pval is None or p_calc <= pval + half + _EPS
    elif op in ('>', '>='):
        consistent = pval is None or p_calc >= pval - half - _EPS
    else:  # '='
        if pval is None:
            ...
            # The reported value's own rounding interval reaches 0 (SPSS
            # prints tiny ps as '0.000'): treat it as the printed interval
            # [0, pval + half), not an exact zero for a ratio band.
            consistent = 0.0 <= p_calc < pval + half + _EPS
        else:
            ratio = max(p_calc, 1e-20) / max(pval, 1e-20)
            consistent = 0.1 < ratio < 10

    # Significance-decision flip across alpha = .05.  The recomputed side
    # gets the reported value's half-ulp as slack around alpha: a p_calc
    # within display rounding of .05 (e.g. 0.0501 against '<0.05') is
    # ambiguous, not a flip — symmetric with the consistency slack above.
    claims_sig = _reported_claims_significance(op, pval)
    if p_calc < ALPHA - half - _EPS:
        calc_sig = True
    elif p_calc > ALPHA + half + _EPS:
        calc_sig = False
    else:
        calc_sig = None
    decision_flip = (claims_sig is not None and calc_sig is not None
                     and claims_sig != calc_sig)

    if consistent and not decision_flip:
        verdict, note = 'PASS', 'consistent within rounding'
    elif decision_flip:
        verdict = 'FLAG'
        note = ('significance decision at alpha = .05 flips '
                '(reported vs recomputed)')
    else:
        verdict, note = 'FLAG', 'inconsistent (reporting error)'

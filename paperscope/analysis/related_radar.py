...
) -> "openalex_client.OpenAlexResult":
    """Search OpenAlex for works matching a query.

    Returns the typed :class:`OpenAlexResult` so the caller can tell an API
    outage (``network_error``) apart from a genuine empty result set — the old
    ``except RequestException: return []`` collapsed the two.
    """
    return openalex_client.get(
        "works",
        {
            "search": query,
            "per_page": min(n, 200),
...
def related_radar(
    ...
    for kw in keywords:
        ...
        resp = _search_openalex(kw, n=n_results // len(keywords) + 5, email=email)
    ...
    if not all_works:
        # Distinguish "OpenAlex was unreachable" from "OpenAlex returned nothing":
        # a caller must not treat an outage as evidence of no missing related work.
        return {
            "error": "OpenAlex unreachable" if network_error else "No results from OpenAlex",

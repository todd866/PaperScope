...
OPENALEX_BASE = openalex_client.OPENALEX_BASE
MAILTO = os.environ.get("PAPERSCOPE_EMAIL", "paperscope@example.com")
RATE_LIMIT_DELAY = openalex_client.RATE_LIMIT_DELAY  # unified polite-pool pace
...
def _oa_get(endpoint: str, params: Optional[Dict] = None) -> Dict:
    """GET from OpenAlex via the shared client; ``{}`` on any failure.

    Kept as a thin adapter (rather than inlining ``openalex_client.get``) so
    ``citation_uptake`` and its tests can keep patching ``author_profile._oa_get``
    and so callers that only care about the happy-path dict are undisturbed.
    """
    resp = openalex_client.get(endpoint, params, email=MAILTO)
    if not resp.ok:
        print(f"  [author_profile] OpenAlex request failed: {resp.error}")
        return {}
    return resp.data

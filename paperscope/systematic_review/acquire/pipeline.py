...
class AcquireResult:
    ...
    shadow_fetched: int = 0          # Phase 1.5 (opt-in): shadow-provider hits
    shadow_no_md5: int = 0           # DOI not found by the shadow provider
    shadow_no_pdf: int = 0           # found in archive but download failed
    shadow_doi_mismatch: int = 0     # shadow candidates: none carried the DOI (collision guard)
    ...
    *,
    ...
    if paywalled:
        ...
        if verbose:
            ...
            # The hinted command must stay a real, parseable invocation —
            # tests/test_sr_acquire_hint.py parses it against the actual CLI.
            print("Run `python -m paperscope.systematic_review browser-harvest "
                  f"--corpus {shlex.quote(str(corpus_dir))}` to walk the tail "
                  "with a real browser profile. Read its --help first: automated "
                  "bulk retrieval through institutional subscriptions can breach "
                  "publisher license terms — pace it conservatively.")

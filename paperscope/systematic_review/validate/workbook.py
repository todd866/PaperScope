...
def _corrected_html(decision_options: list[str]) -> str:
    """The structured corrected-value control: a decision select for
    screening-shaped workbooks, a JSON field editor for extraction-shaped
    ones. Exported as data (`corrected_decision` / `corrected_fields`) so a
    flip carries the human's answer, not just a free-text note."""
    if decision_options:
        opts = "".join(f'<option value="{esc(o)}">{esc(o)}</option>' for o in decision_options)
        return ('<label class="corr">corrected decision (only if flipping): '
                f'<select class="corr-dec"><option value=""></option>{opts}</select></label>')
    return ('<input class="corr-fields" placeholder=\'corrected fields '
            '(JSON object, e.g. {"field": "value"} — only if flipping)\'>')
...
.corr{display:block;font-size:13px;margin-top:8px} .corr select{font:13px inherit;padding:2px 6px}
.corr-fields{width:100%;margin-top:8px;padding:6px 8px;border:1px solid #ccc;border-radius:6px;font:12px ui-monospace,Menlo,monospace}
...
function state(c){
  ...
  const cd=c.querySelector('.corr-dec'), cf=c.querySelector('.corr-fields');
  return {reviewed:c.querySelector('.cb-rev').checked, flip:c.querySelector('.cb-flip').checked,
          ratings, corrected_decision:cd?cd.value:'', corrected_fields:cf?cf.value.trim():'',
...
cards.forEach(c=>{
  ...
  if(raw){ try{ const s=JSON.parse(raw);
    ...
    const cd=c.querySelector('.corr-dec'); if(cd) cd.value=s.corrected_decision||'';
    const cf=c.querySelector('.corr-fields'); if(cf) cf.value=s.corrected_fields||'';
  ...
  const cfEl=c.querySelector('.corr-fields'); if(cfEl) cfEl.addEventListener('input',()=>save(c));
});
function exportState(){
  const out={};
  cards.forEach(c=>{ const s=state(c);
    if(s.reviewed||s.flip||s.note||s.corrected_decision||s.corrected_fields||Object.keys(s.ratings).length)
      out[c.dataset.rid]=s; });
...
def build_workbook(decisions: list[dict], records_by_id: dict[str, dict], rubric: FrictionRubric,
                   ...
                   wb_id: str = "wb", decision_options: list[str] | None = None) -> str:
    """Return the workbook HTML. `self_audit` maps record_id -> audit dict.

    `decision_options` drives the structured corrected-value control: the
    values a flipped decision can be corrected to (a select per card). Default
    is include/exclude/maybe plus any observed decision values when the rows
    are screening-shaped (have a `decision` key); for extraction-shaped rows a
    corrected-fields JSON input is rendered instead. The export carries
    `corrected_decision` / `corrected_fields` so `reconcile` gets the human's
    corrected value as data, not just a note."""
    self_audit = self_audit or {}
    if decision_options is None:
        if any("decision" in d for d in decisions):
            decision_options = ["include", "exclude", "maybe"]
            observed = {str(d["decision"]) for d in decisions if d.get("decision")}
            decision_options += sorted(observed - set(decision_options))
        else:
            decision_options = []
    corrected_ctl = _corrected_html(decision_options)
...
<div class="wrap">
<p style="color:#555;font-size:14px;margin:18px 0">AI-flagged low-confidence decisions are at the top. Rate the friction dimensions, tick <b>reviewed</b> when you agree, <b>flip / disagree</b> + set the <b>corrected value</b> (and optionally a note) when you would change the decision — the corrected value is what the pipeline adopts as the decision-of-record. State saves in your browser. When done, hit <b>Copy review (JSON)</b> and paste it back for <code>validate reconcile</code>.</p>

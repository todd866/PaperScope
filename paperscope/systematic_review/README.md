...
> **Caution — institutional access.** `browser-harvest` automates bulk PDF
> retrieval through your institution's subscriptions. That can breach
> publisher license terms and, in the worst case, get the *institution's*
> access suspended. It is the operator's call whether their agreement covers
> it: keep concurrency conservative, pace with `--inter-paper-delay`, and see
> the caution section in `docs/systematic-review.md` before running at scale.
...
| `acquire.browser_driver` | ✅ | Playwright-driven institutional-access harvester for the paywalled tail. Supports real Chrome profiles, warmup DOI, publisher grouping, strategy cache, and inter-paper pacing. CLI: `browser-harvest` — read the license-terms caution above and in its `--help` before bulk runs. |
| `validate` | ✅ | Human-in-the-loop adjudication: self-audit-sorted HTML workbook, append-only reconcile to `validation-overrides.jsonl`, and `validate.effective` loaders that fold overrides into the decisions-of-record consumed by `prisma`/`aggregate`/`build-site`. See `docs/validate.md`. |
...
| `knowledge_base` | ✅ | KB-bundle exporter (roadmap §6): a review corpus dir → `paper-cards.jsonl` (one card per included record: identity + screening decision-of-record + charted fields + quality flags) + `clusters.json` (cards grouped by a configurable charted field) + `manifest.json` (counts, provenance, schema version). Routes screening/extraction through `validate.effective`, so human overrides are the decisions-of-record. Pure functions + `knowledge-base` CLI. Generic — caller supplies card/cluster/quality fields. See `../../docs/corpus-knowledge-base.md`. |
| `rater_compare` | ✅ | Field-level disagreement between two raters' screening/extraction JSONL (roadmap item 3): per-field agreement counts + percent agreement, Cohen's kappa for a categorical field (Cohen 1960), and a per-record disagreement listing for adjudication. Handles missing records, list-valued fields (set comparison), and case/whitespace normalization. CLI: `rater-compare` (JSON + readable table). |
| `knowledge_base` (rich layers) | 🧭 | Still roadmap: source-object manifests, public summaries, private source links, cluster-level summaries, and paper-in-corpus evaluation (nearest neighbours, claim links). See `../../docs/corpus-knowledge-base.md` §1–5. |
...
The first two of those modules are now shipped:

```bash
# Export a self-contained KB bundle (paper-cards.jsonl + clusters.json + manifest.json)
python -m paperscope.systematic_review knowledge-base \
  --corpus ./corpus --out ./kb --cluster-field study_design

# Field-level disagreement between two rater passes over the same records
python -m paperscope.systematic_review rater-compare \
  --rater-a rater1/screening.jsonl --rater-b rater2/screening.jsonl \
  --kappa-field decision --out compare.json --table
```

The exporter routes screening/extraction through `validate.effective`, so
human adjudications are the decisions-of-record on every card. The richer
layers (source-object manifests, cluster summaries, paper-in-corpus
evaluation) remain roadmap.

...
What v0 emits:
  ...
  index.html                — funnel summary, exclusion-reason breakdown,
                              ...
                              picture in one page)
  screening/<decision>.html — one list page per decision category
                              (include/exclude/maybe), linked from the index
  record/<pmid>.html        — one page per screened record showing title,
                              abstract, journal/year, the decision-of-record
                              (human validation overrides win over the AI
                              decision, with provenance) + reason, and the
                              rubric themes hit. Linked from the lists.
...
def _safe_name(value: str) -> str:
    """Sanitize a data-derived value for use as an HTML file stem.

    Decision strings and pmids come from JSONL data; a value containing a
    path separator (e.g. a pmid of '../evil') must never navigate out of
    the site directory. Identity for ordinary pmids/decisions."""
...
def _render_index(stats: dict, linked_decisions: set[str] | None = None) -> str:
    rows = "".join(
    ...
    # Only decisions whose list page was actually written become links;
    # anything else (unexpected decision values) shows as a plain count.
    linked = linked_decisions if linked_decisions is not None else set(stats["by_decision"])
    decisions = "".join(
        (
            f"<a href='screening/{html.escape(_safe_name(d))}.html' class='tag {_decision_class(d)}'>"
            f"{html.escape(d)} ({n})</a> "
        )
        if d in linked
        else f"<span class='tag {_decision_class(d)}'>{html.escape(d)} ({n})</span> "
...
def _render_decision_list(decision: str, items: list[tuple[dict, dict]]) -> str:
    # Decision-list pages live under screening/; record pages under record/.
    rows = "".join(
        f"<tr><td><a href='../record/{html.escape(_safe_name(r['pmid']))}.html'>{html.escape(r['pmid'])}</a></td>"
...
def _render_record(record: dict, decision: dict) -> str:
    ...
    if decision.get("decided_by") == "human":
        note = decision.get("human_note") or ""
        note_html = f" — {html.escape(note)}" if note else ""
        override_html = (
            f"<p class='reason'>human override (AI said "
            f"{html.escape(str(decision.get('ai_decision') or '?'))}){note_html}</p>"
        )
    body = (
        f"<nav class='crumb'><a href='../index.html'>&larr; index</a></nav>"
...
def build_review_site(
    ...
    *,
    ...
    # Decisions-of-record: human validation overrides (if any) folded over the
    # AI screening JSONL — the site must show what the review actually decided.
    from paperscope.systematic_review.validate.effective import load_effective_screening

    screening = load_effective_screening(corpus_dir)
    ...
    for d in screening:
        ...
        if decision in grouped:
            ...
            (out_dir / "record" / f"{_safe_name(r.get('pmid',''))}.html").write_text(
    ...
    (out_dir / "index.html").write_text(
        _render_index(stats, linked_decisions=set(grouped)), encoding="utf-8"
    )
    for decision, items in grouped.items():
        (out_dir / "screening" / f"{_safe_name(decision)}.html").write_text(

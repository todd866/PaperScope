...
def _build_card(ref: dict) -> str:
    ...
    # Only open/legal discovery links are included. Last-resort ("shadow")
    # acquisition is NOT part of the public build; bring your own provider —
    # see docs/ACQUISITION.md.
    links = []
    if doi:
...
def generate_sourcing_page(
    ...
    Args:
        ...
        skip_own_unpublished: Exclude own unpublished papers (in press /
            submitted / accepted). Only takes effect when ``own_key_prefix``
            identifies which cite keys are the author's own.
        own_key_prefix: Cite-key prefix marking the author's own entries (e.g.
            ``"todd"`` for keys like ``todd2024method``). ``None`` (default)
            disables the own-unpublished skip entirely — callers can derive a
            prefix from the bib's own-author field. Baking a fixed surname in
            here silently no-ops for every other user.
    ...
    for r in bib["references"]:
        ...
        if skip_web_only and any(
            ...
            skip_keys.add(key)
        if (
            skip_own_unpublished
            and own_key_prefix
            and key.startswith(own_key_prefix)
            and r.get("note", "").lower() in ("accepted", "in press", "submitted")
        ):
            skip_keys.add(key)
...
def sourcing_page_main(
    ...
    if output is None:
        # Write next to the input data_dir, not a machine-specific ~/Desktop
        # (which doesn't exist on headless/Linux boxes and isn't the user's).
        output = data_dir / f"{tex_file.stem}_papers_to_source.html"

...
│                       │                                             │
│                       ▼                                             │
│                 analysis/  (21 modules: citation alignment,         │
│                  novelty, journal fit, critical read, forensic,     │
│                  reported-stats text mode, calibration gate …)      │
└──────────────────────────────┬──────────────────────────────────────┘
                               │
                               ▼
┌─────────────────────────────────────────────────────────────────────┐
...
> **Acquisition boundary:** Last-resort ("shadow") acquisition is NOT included in the public build; bring your own provider — see `docs/ACQUISITION.md`. Open-access acquisition (Unpaywall / publisher OA / EZProxy) is unaffected. See `docs/acquisition-lessons.md` for the general lesson that a downloaded PDF is not automatically a trustworthy one, and the verification-gated cascade concept.
...
3. **Analysis** (`analysis/`), 21 modules in three groups (a `cli.py` dispatcher and a
   shared `_common.py` sit alongside them). These modules can run on one manuscript or one
   external paper, but their strongest form uses a corpus as context:
   ...
   *Embedding-powered (your own papers)*
   ...
   - `citation_uptake` — independent (non-self) citation uptake of a DOI (via OpenAlex)
   ...
   *Critical read (external papers)*
   ...
   - `audit_router` — study-type classifier routing to the right validity battery
   - `annotate` — PDF + notes spec → annotated reading copy

   *Forensic statistics (data integrity)*
   - `forensic_stats` — 22 checks based on Heathers (2025): GRIM, GRIMMER, GRIM-for-percentages (`grim_percentage`; `debit` is a deprecated alias, **not** the real DEBIT of Heathers & Brown 2019, which is unimplemented), SPRITE, correlation bounds, t-test/ANOVA/chi-squared recalculation, Carlisle-Stouffer-Fisher, SD/SE confusion, Benford's law, variance ratios, effect size consistency, and more. GRIM/GRIMMER/SPRITE test round-half and truncation (`convention="both"`) so honest rounding never yields a false accusation.
   - `forensic_report` — shared Finding/Report contract + table-mode check runner (JSON table spec → Report)
   - `reported_stats` — statcheck-style t/F/chi²/r/z extraction + interval-safe p recalculation (text mode)
   - `calibration` — forensic regression gate: known-answer cases measuring sensitivity + specificity of the checks
...
5. **Validate** (`validate/`): Human-in-the-loop adjudication of AI screening/extraction. `workbook` renders decisions + self-audit into a scroll-through HTML queue; `reconcile` folds the human's export back as append-only validation-overrides; `summary` reports calibration. `validate/effective.py` resolves the effective decision for every record so **human overrides are the decision-of-record** — every downstream reader (synthesis, knowledge base) routes through it.
6. **Synthesise** (`synthesise/`): Declarative aggregator (`aggregate.py`) + PRISMA-ScR flow (`prisma.py`) + cross-database dedup. Regression-verified against a working scoping review.
7. **UI** (`ui/`): Static HTML review site with Covidence-style record pages. No JS dependency — publishable as a `gh-pages` artefact.

Two audit subpackages sit alongside these: `forensic_scan/` (batch forensic checks over a corpus) and `methodological_audit/` (study-type methodological audit).

See `docs/systematic-review.md` for the design + roadmap and `paperscope/systematic_review/README.md` for the quickstart.

### Phase 5: Corpus Knowledge Bases (first modules shipped)
...
The first two modules have shipped as `systematic_review` subcommands:

1. **Paper cards + clusters** (`knowledge_base/`, `knowledge-base` subcommand — **shipped**): a corpus directory exports a self-contained bundle — `paper-cards.jsonl` (per-paper summaries with relevance, methods, quality flags, claim links), `clusters.json` (grouping by a charted field), and `manifest.json`. Screening/extraction route through `validate/effective` so human overrides are the decision-of-record.
2. **Rater comparison** (`rater_compare.py`, `rater-compare` subcommand — **shipped**): field-level agreement/disagreement across two rater passes, percent agreement, Cohen's kappa on a categorical field, and a per-record disagreement listing for adjudication.

Still on the roadmap:

3. **Source manifests**: Provider-neutral object metadata for PDFs and extracted text, with checksums and public/private access flags (a `manifest.json` skeleton ships; richer source-object links are pending).
4. **Cluster maps**: Embedding-derived clusters and cluster-level summaries beyond single-field grouping.
5. **Export**: App-ready packages and a searchable collaborator portal beyond the static bundle.

See `docs/corpus-knowledge-base.md` for the detailed roadmap.

### Adjacent packages (cross-cutting)

Two packages sit outside the five-phase pipeline but support it:

- **`net/`** — shared HTTP clients. `net/openalex_client.py` is the single polite-pool OpenAlex client that the analysis (`journal_targeting`, `related_radar`, `citation_uptake`) and harvest layers call instead of each re-implementing request/retry/rate-limit handling.
- **`paper_site/`** — a native Next.js paper-reader scaffold behind the `paper-site` command. It emits a paper-library-backed manuscript reader with a shared MD3 visual contract: native web manuscript first, downloadable PDFs second, side-panel reference context.
